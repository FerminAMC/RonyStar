class Bullet{
  ...
  boolean hit(Enemy e){
    ...
    if(this.position.x + this.icon.width/2 >= width || this.position.x <= 0){
      ...
      if(this.position.x + this.icon.width/2 >= -offset+e.position.x - e.image.width/2 && // Cambio de signo
        this.position.x - this.icon.width/2 <= -offset+e.position.x + e.image.width/2 && // Cambio de signo
